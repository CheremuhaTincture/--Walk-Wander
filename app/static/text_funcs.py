...
async def change_text_lobby_to_gameplay(__key, _text, message: Message):
    message_n_chat_ids = await rq.get_main_message_ids_n_index(__key)

    turn = await rq.get_turn(__key)

    for id in message_n_chat_ids:
        try:
            if int(id.split('_')[2]) == turn:
                await message.bot.edit_message_text(message_id=id.split('_')[0],
                                                    chat_id=id.split('_')[1],
                                                    text=_text,
...
"""async def send_text_gameplay(__key, _text, callback: CallbackQuery):
    ...
    for id in message_n_chat_ids:
        try:
            await message.bot.delete_message(message_id=id.split('_')[0],
                                                chat_id=id.split('_')[1])
            if id.split('_')[2] == '0':
                msg = await message.bot.send_message(chat_id=id.split('_')[1],
                                                    text=_text,
                                                    reply_markup = kb.gameplay_menu)
            else:
                msg = await message.bot.send_message(chat_id=id.split('_')[1],
                                                    text=_text)
            await rq.set_main_message(callback.from_user.id, __key, msg.message_id)
        except ae.TelegramBadRequest:
            continue"""
